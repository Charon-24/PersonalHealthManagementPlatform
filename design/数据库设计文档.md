...
| `deleted` | `tinyint(1)` | 否 | 否 | 软删除（0 正常 / 1 已删除） |
...
| `permissions` | `varchar(1000)` | 是 | 否 | 权限标识 JSON 数组 |
| `create_time` | `datetime(6)` | 否 | 否 | 创建时间 |
| `update_time` | `datetime(6)` | 是 | 否 | 更新时间 |
...
| `abnormal_flag` | `tinyint(1)` | 是 | 否 | 异常标记（0 正常 / 1 异常） |
| `abnormal_remark` | `varchar(255)` | 是 | 否 | 异常备注 |
| `deleted` | `tinyint(1)` | 否 | 否 | 软删除（0 正常 / 1 已删除） |
| `create_time` | `datetime(6)` | 否 | 否 | 创建时间 |
...
| 字段 | 类型 | 可空 | 主键 | 说明 |
| --- | --- | --- | --- | --- |
| `id` | `bigint` | 否 | 是 | 报告 ID（自增） |
| `user_id` | `bigint` | 否 | 否 | 用户 ID |
| `period_type` | `varchar(20)` | 否 | 否 | 报告周期（week/month/quarter） |
| `start_date` | `date` | 否 | 否 | 统计起始日期 |
| `end_date` | `date` | 否 | 否 | 统计结束日期 |
| `health_score` | `int` | 是 | 否 | 健康评分（0-100） |
| `summary` | `text` | 是 | 否 | 报告摘要 |
| `health_analysis` | `text` | 是 | 否 | 健康数据分析 |
| `sport_analysis` | `text` | 是 | 否 | 运动数据分析 |
| `diet_analysis` | `text` | 是 | 否 | 饮食数据分析 |
| `suggestions` | `text` | 是 | 否 | 改善建议 |
| `export_url` | `varchar(255)` | 是 | 否 | PDF 导出地址 |
| `deleted` | `tinyint(1)` | 否 | 否 | 软删除（0 正常 / 1 已删除） |
| `create_time` | `datetime(6)` | 否 | 否 | 生成时间 |
...
7. 外键约束：`health_report.user_id -> user.id`。
...
6. `health_report(user_id, create_time)`。
...
3. ~~是否增加软删除字段（如 `deleted`）~~ **已确认：全局启用软删除。**
